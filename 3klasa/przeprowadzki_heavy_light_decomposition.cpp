...
set<int> path[roz]; //set z pozycjami zablokowanymi na sciezce //sciezki w drzewie ida z gory na dol
...
int renum[roz]; //zwraca numer wierzcholka po jego poziomie
...
bool walk(int x,int p){ //czy da sie dojsc z punktu a do p przodka
    int S=num[x];
    if(S==num[p]){
    ...
    auto it_down=path[S].lower_bound(poz[ pie_na_s[S] ]);
    if(it_down==path[S].end() or (*it_down)>poz[x] )return walk(ojciec[pie_na_s[S]],p);
    else return false;
}
int main(){
int Z;
