...
//zrzut jest wektorem do ktorego to wsadza
 ...
 int centroid(int n,int x){ //wywoluje sie na grafie i znajduje jego centroid
    ...
    int wie=x;
    while(true){
        bool br=0;//czy powiniem skonczyc //0 tak
        int maksy;
        for(int i=0;i<T[wie].size();i++){
           pair<int,int> s=T[wie][i];
            if(czarny[s.st])continue;
            if(ile_dzieci[s.st]+1>graf_roz/2){br=1;maksy=s.st;break;}
        }
        if(br==0)break;
        ile_dzieci[wie]=graf_roz-ile_dzieci[maksy]-1 -1;
        wie=maksy;
        }
